...
# Clasificación de Fases del Ciclo Menstrual mediante Redes Híbridas CNN-LSTM en Series Temporales
...
**Integrantes:** <br> Sofía Roca Contreras <br> Bianca Bruna Ramírez <br> **Grupo:** <br> DeepFlow
:::
...
## Aspectos relevantes de la presentación anterior
...
| **CNN 1D** [@khairunisa2025comparison] | **88.9%** | Buena: Identifica patrones clave, pero es inferior a LSTM en secuencias largas. |
...
- Implementar modelos LSTM y CNN 1D para la clasificación de las fases del ciclo menstrual.

- Comparar su desempeño con un modelo tradicional mediante F1-Score y métricas por fase.

## Hipotesis {.smaller}

Una arquitectura híbrida de Deep Learning (1D-CNN + LSTM Many-to-One), que integra el contexto temporal de los ciclos de las usuarias, superará a un modelo clásico de Machine Learning (Random Forest) en la tarea de clasificación multiclase de las fases del ciclo menstrual, logrando un incremento en el Macro F1-Score sobre el conjunto de test.
...
3.  Algoritmo Predictivo Multivariado (**MICE - Predictive Mean Matching**) [@van2011mice]: edad, imc, intensidad_sangrado_media, promedio_duracion_menstruacion, promedio_duracion_ciclo.

4.  Imputación global (**Media global**) [@logapriya2024optimized]: intensidad_sangrado_media, duracion_menstruacion, duracion_ciclo, primer_dia_alta_fertilidad, total_dias_alta_fertilidad, total_dias_peak_fertilidad, dia_estimado_ovulacion, duracion_fase_lutea.
...
Tranformar a base de datos diaria e identificar a que ciclo pertenece cada registro diario. [@clark2018open]
...
## Código red híbrida CNN-1D y LSTM {.smaller}
...
## Estructuración Temporal: Ventanas Deslizantes {.smaller}

Agrupación de datos en bloques de una semana para capturar el contexto fisiológico inmediato. [@gulati2026menstrual]
...
| **LSTM**  | [@harshavardhan2025ai] | 90% | - | -  | - | 89% |
| **CNN 1D** | [@khairunisa2025comparison] | 87.5% | -  | - | - | 87.5% |
...
* Se logró desarrollar e implementar exitosamente una arquitectura híbrida (CNN-1D + LSTM) capaz de procesar historiales menstruales diarios.

* Se demostró que un modelo tradicional como Random Forest cae en la "Trampa de la Exactitud", fallando en detectar la ventana ovulatoria (F1 de 37.1%). Nuestra red híbrida aprendió la dependencia temporal, elevando esta detección (F1 de 50.7%) y logrando el mejor Macro F1-Score global (77.1%).

* El uso de Deep Learning permite predecir fases adaptándose a la variabilidad biológica real de mujeres con ciclos irregulares.

## Limitaciones y trabajo futuro {.smaller}

### Limitaciones

* La base actual no cuenta con registros continuos de Temperatura Basal (BBT) ni niveles hormonales (LH/FSH), los cuales son fisiológicamente relevantes para determinar el día exacto de la ovulación. [@clark2018open]

*  Los datos provienen de registros manuales, lo que introduce ruido y subjetividad humana en variables como "intensidad del sangrado".

### Trabajo Futuro

* Integrar datos continuos provenientes de dispositivos wearables (relojes inteligentes, anillos) que midan variabilidad de la frecuencia cardíaca y temperatura de la piel durante el sueño. [@khadse2025systematic]

* Probar mecanismos de Atención (Transformers) [@logapriya2024optimized] para que la red pondere automáticamente qué ciclos históricos son más relevantes para predecir el ciclo actual.

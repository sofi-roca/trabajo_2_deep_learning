...
- Implementar modelos LSTM y CNN 1D para la clasifación de las fases.
...
1.  Relleno secuencial (**Forward fill**): edad, imc, promedio_duracion_ciclo, promedio_duracion_menstruacion, intensidad_sangrado_media.

2.  Imputación local (**Media y mediana historica**): promedio_duracion_ciclo (se recalcula con duracion_ciclo), promedio_duracion_menstruacion (se recalcula con duracion_menstruacion).
...
grafico_desbalance <- ggplot(df_resumen_clases, aes(x = nombre_clase, y = cantidad, fill = nombre_clase)) +
  ...
  labs(
    title = "Desbalance Fisiológico de Clases",
    ...
    caption = "Evidencia de desbalance extremo: La clase minoritaria requiere estrategias de Oversampling."
  ) +
...
grafico_evolucion <- ggplot(ejemplo_ciclo, aes(x = dia_ciclo, y = 1, fill = Fase)) +
  ...
  scale_fill_manual(values = c("Folicular / Menstrual" = "#B4C6E7", 
                               "Ventana Ovulatoria"  = "#FFC000", 
                               "Fase Lútea"          = "#C6E0B4")) +
  ...
  labs(
    title = paste("Progresión del Ciclo y Ventana Deslizante (Paciente:", ejemplo_id, ")"),
    subtitle = "Lo que la red LSTM 've' para intentar predecir el día siguiente",
    x = "Día del Ciclo",
    y = "",
...
* **Transformación a Tensor 3D**: Definir longitud de ventana $L = 7$ días y desplazamiento de 1 día (stride = 1).

* **Split por Usuaria**: La división 80% Traint / 20% Test se realiza estrictamente a nivel de ***id***.

* **Desbalance de clases**: La clase 1 (ovulación) es minoritaria (~3 días por ciclo). Se aplica **oversampling aleatorio** únicamente sobre el conjunto de entrenamiento.
...
| **Tiempo de cómputo**  | Aprox. 1-2 min.            |
